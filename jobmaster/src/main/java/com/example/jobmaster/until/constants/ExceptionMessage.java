package com.example.jobmaster.until.constants;

public class ExceptionMessage {
    public static final String USER_NOT_FOUND = "user-not-found";
    public static final String PASSWORD_INVALID = "Mật khẩu hiện tại không hợp lệ!";
    public static final String USER_NOT_ACTIVE = "user-not-active";
    public static final String CAMPAIGN_NOT_FOUND = "Chiến dịch không tồn tại";
    public static final String ENTERPRISE_NOT_FOUND= "Doanh nghiệp không tồn tại";
    public static final String ENTERPRISE_NOT_ACTIVE = "Doanh nghiệp chưa hoạt động";
    public static final String POST_NOT_FOUND = "Tin không tồn tại";
}
